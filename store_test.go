...
func TestIsKey(t *testing.T) {
	...
	for label, tc := range testCases {
		t.Run(label, func(t *testing.T) {
			assert.Equal(t, tc.Expected, dynamo.IsKey(tc.Key))
		})
	}
}
...
func TestVersionFromKey(t *testing.T) {
	...
	for label, tc := range testCases {
		t.Run(label, func(t *testing.T) {
			version, at, err := dynamo.VersionAndAt(tc.Key)
...
func TestSave(t *testing.T) {
	...
	_, err := api.CreateTable(dynamo.MakeCreateTableInput(tableName, 10, 10))
	if err != nil {
	...
	for label, tc := range testCases {
		t.Run(label, func(t *testing.T) {
			...
			store, err := dynamo.New(tableName,
				dynamo.WithEventPerItem(tc.EventsPerItem),
				dynamo.WithDynamoDB(api),
			)
			assert.Nil(t, err)
